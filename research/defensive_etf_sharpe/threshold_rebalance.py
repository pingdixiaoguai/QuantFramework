...
def _trigger_value(
    ...
    if spec.kind in ("max_asset_drift", "calendar_or_max_asset_drift"):
        return max(
            [abs(actual_weights.get(asset, 0.0) - current_target.get(asset, 0.0)) for asset in assets]
            + [actual_cash_weight]
        )
    if spec.kind in ("target_change", "calendar_or_target_change"):
        return 0.5 * sum(
    ...
    *,
    ...
    for day_index, timestamp in enumerate(data.dates):
        ...
        if trigger.kind == "calendar_monthly":
            ...
            if trigger.kind.startswith("calendar_or_") and deposit_dates.get(month) == timestamp:
                value = 1.0
