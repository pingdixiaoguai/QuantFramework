"""Staged ETF-specific X/Y search for the formal W40 Top1 escape."""
...
DEFAULT_CONFIG = Path(
    "research/configs/momentum_defender_w40_asset_specific_escape_search.yaml"
)
DEFAULT_OUTPUT = Path(
...
def _load(path: Path) -> dict:
    ...
    if not isinstance(value, dict):
        raise ValueError("asset-specific escape config must be a mapping")
    return value


def _policies(config: dict) -> list[AssetXYPolicy]:
    values = config["threshold_grid"]
    result = {}
    for x, y in product(values["entry_x"], values["exit_y"]):
        if float(y) > float(x):
            continue
        policy = AssetXYPolicy(float(x), float(y))
...
def _single_neighborhood(table: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    for asset, group in table.groupby("asset", sort=False):
        coords = np.column_stack(
            [
                group["entry_x"].map(x_lookup).to_numpy(int),
                group["exit_y"].map(y_lookup).to_numpy(int),
            ]
        )
        arrays = {
...
def _rank_select(
    ...
    pool = table.loc[mask].copy()
    if pool.empty:
        return []
    ranks = pool[fields].rank(pct=True)
    pool["rank_min"] = ranks.min(axis=1)
    pool["rank_mean"] = ranks.mean(axis=1)
    return list(
        pool.sort_values(
            ["rank_min", "rank_mean", fields[0], fields[1]], ascending=False
        ).head(count).index.astype(str)
    )


def _select_single_options(
    table: pd.DataFrame,
    policy_lookup: dict[str, AssetXYPolicy],
    config: dict,
) -> dict[str, list[AssetXYPolicy | None]]:
    staged = config["staged_selection"]
    count = int(staged["policies_per_asset"])
    fields_joint = [
    ...
    for asset in MOMENTUM_ASSETS:
        asset_rows = table["asset"].eq(asset)
        eligible = (
            asset_rows
        ...
        for fields in (
            ...
            ["full_sharpe", "full_annualized_return_252"],
            ["ordinary_annualized_return_252", "ordinary_sharpe"],
            fields_joint,
        ):
            for candidate_id in _rank_select(table, eligible, fields, count=1):
                if candidate_id not in ids:
                    ids.append(candidate_id)
        if len(ids) < count:
            for candidate_id in _rank_select(
                table, eligible, fields_joint, count=count
            ):
                if candidate_id not in ids:
                    ids.append(candidate_id)
                if len(ids) == count:
                    break
        policies = [policy_lookup[str(table.at[candidate_id, "policy_id"])] for candidate_id in ids[:count]]
        result[asset] = [None, *policies]
    return result
...
def _joint_neighborhood(table: pd.DataFrame) -> pd.DataFrame:
    ...
    for position, candidate_id in enumerate(table.index):
        members = np.sum(coords != coords[position], axis=1) <= 1
        summaries[str(candidate_id)] = {
            "neighborhood_count": int(members.sum()),
            **{
                f"neighborhood_{name}_q25": float(
...
def _joint_select(
    ...
    pool = table.loc[mask].copy()
    if pool.empty:
        raise RuntimeError("joint asset-specific selection pool is empty")
    ranks = pool[fields].rank(pct=True)
    pool["rank_min"] = ranks.min(axis=1)
    pool["rank_mean"] = ranks.mean(axis=1)
    return pool.sort_values(
        ["rank_min", "rank_mean", "enabled_assets", fields[0], fields[1]],
...
def run_experiment(root: Path, config_path: Path, output: Path) -> dict:
    ...
    if frozen["formal_strategy_id"] != FORMAL_STRATEGY_ID:
        raise AssertionError("asset-specific search is not pinned to formal strategy")
    ...
    if not (
        ...
        raise AssertionError("fixed asset-specific mechanism mismatch")
    ...
    for asset in MOMENTUM_ASSETS:
        for policy in policies:
            ...
            run = run_asset_specific_w40_escape(
                context, formal.state, policy_set, metrics=metrics_at_open
            )
    ...
    options = _select_single_options(single_table, policy_lookup, config)
    ...
    for selected in product(*ordered):
        ...
        candidate_id = policy_set_id(policy_set)
    ...
    off_id = policy_set_id({asset: None for asset in MOMENTUM_ASSETS})
    disabled_parity = float(
        (combo_returns_frame[off_id].astype(float) - baseline).abs().max()
    )
    if disabled_parity > 1e-14:
        raise AssertionError("disabled asset-specific policies do not match baseline")
    ...
    current.columns = [f"asset_xy::{column}" for column in current]
    ...
    selected_policy_set = combo_policies[selected_id]
    leave_fixed_rows = []
    yearly_fixed_rows = []
    for year in sorted(selected_returns.index.year.unique()):
        keep = selected_returns.index.year != year
        candidate_leave = performance(selected_returns.loc[keep])
        baseline_leave = performance(baseline.loc[keep])
        leave_fixed_rows.append(
            {
                "removed_year": int(year),
                "candidate_annualized_return_252": candidate_leave[
                    "annualized_return_252"
                ],
                "formal_annualized_return_252": baseline_leave[
                    "annualized_return_252"
                ],
                "candidate_sharpe": candidate_leave["sharpe"],
                "formal_sharpe": baseline_leave["sharpe"],
            }
        )
        year_mask = selected_returns.index.year == year
        candidate_year = selected_returns.loc[year_mask]
        baseline_year = baseline.loc[year_mask]
        yearly_fixed_rows.append(
            {
                "year": int(year),
                "candidate_return": float((1.0 + candidate_year).prod() - 1.0),
                "formal_return": float((1.0 + baseline_year).prod() - 1.0),
                "candidate_sharpe": performance(candidate_year)["sharpe"],
                "formal_sharpe": performance(baseline_year)["sharpe"],
            }
        )
    leave_fixed = pd.DataFrame(leave_fixed_rows)
    yearly_fixed = pd.DataFrame(yearly_fixed_rows)
    leave_fixed.to_csv(output / "selected_fixed_leave_one_year.csv", index=False)
    yearly_fixed.to_csv(output / "selected_fixed_calendar_years.csv", index=False)
    selected_config = {
        "strategy_id": "momentum_defender_w40_asset_specific_qm20_escape_candidate_v1",
        "status": (
        ...
        "policies": {
            ...
            for asset, policy in selected_policy_set.items()
        },
    ...
    audit = {
        ...
        "selected_fixed_leave_one_year": {
            "candidate_min_annualized_return_252": float(
                leave_fixed["candidate_annualized_return_252"].min()
            ),
            "candidate_min_sharpe": float(
                leave_fixed["candidate_sharpe"].min()
            ),
            "annualized_beats_formal_rate": float(
                leave_fixed["candidate_annualized_return_252"]
                .gt(leave_fixed["formal_annualized_return_252"])
                .mean()
            ),
            "sharpe_beats_formal_rate": float(
                leave_fixed["candidate_sharpe"]
                .gt(leave_fixed["formal_sharpe"])
                .mean()
            ),
        },
        "selected_calendar_years": {
            "return_win_rate": float(
                yearly_fixed["candidate_return"].gt(yearly_fixed["formal_return"]).mean()
            ),
            "sharpe_win_rate": float(
                yearly_fixed["candidate_sharpe"].gt(yearly_fixed["formal_sharpe"]).mean()
            ),
        },
        "joint_cscv": {
            "full": cscv_summary,
