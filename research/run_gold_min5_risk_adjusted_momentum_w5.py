"""Search thresholds for registered 5-day risk-adjusted momentum Gold escape."""
...
DEFAULT_CONFIG = Path(
    "research/configs/gold_min5_risk_adjusted_momentum_w5_search.yaml"
)
DEFAULT_OUTPUT = Path(
    "experiments/20260823_gold_min5_risk_adjusted_momentum_w5"
)
...
def _params(row: pd.Series) -> GoldRAQMW5Params:
    return GoldRAQMW5Params(
...
def run_experiment(root: Path, config_path: Path, output: Path) -> dict[str, object]:
    ...
    runs = {name: run_gold_raqm_w5(context, _params(row)) for name, row in winners.items()}
    ...
    latest_metrics = risk_adjusted_momentum_at_open(context.curves, window=5).iloc[-1]
    summary = {
    ...
    report = f"""# 5日注册风险调整动量：黄金硬持有5日

## 固定口径

Gold和Defender整体NAV均调用项目注册的`risk_adjusted_quality_momentum`：5日对数收益除以
带8%年化波动率地板的5日波动，再乘Kaufman路径效率并裁剪。黄金前5个完整交易日硬持有，
...
该实验对齐注册风险调整动量，只改变窗口为5日并搜索两个阈值；不会自动修改生产C2。
"""
