...
Each module directory (`data/`, `factors/`, `standardization/`, `strategy/`, `execution/`, `notification/`, `backtest/`) has its own `AGENTS.md` containing that module's contract, implementation notes, known deviations, and pitfalls. **These are the authoritative source for module-level context — do not look for deviation notes in this root file.**

**IMPORTANT**: When modifying a module's code, always check if the same directory's `AGENTS.md` needs updating. Specifically:
...
If unsure whether an update is needed, read the subdirectory `AGENTS.md` and verify each statement still holds against the code you just changed.
