...
- `rolling_ohlc_er.py` is research/training code for the 4ETF OHLC-path parameter search. It keeps quarterly OHLC path weights, rolls them forward using the prior 1008 union-calendar trading dates, searches ±0.05 around the previous quarter at 0.01 steps, and averages the training-Sharpe Top10. The daily runner does not import it; it reads the versioned `quality_momentum_top1_ohlc_er.yaml` checkpoint through the standard factor/strategy pipeline.
...
- The OHLC ER checkpoint is versioned in `strategy/configs/quality_momentum_top1_ohlc_er.yaml`, including effective date, training range, history, selection method, and post-adjusted price basis. Daily runs must not write rolling parameter caches or perform parameter search.
