...
- `runner.py` — day-by-day traversal over the union of all assets' trading days
  ...
  - **Transaction cost** (`config["commission_ratio"]`, float, default `0.0`): on each open execution the engine deducts `commission_ratio * turnover` from that day's return, where `turnover = Σ|Δweight|` across held assets (a full Top1 switch A→B = 2.0, a first entry from cash = 1.0 — charged on both buy and sell legs). Applied multiplicatively via `_turnover()`, so it also registers a negative return on a trade day that has no market return. Default `0.0` keeps historical cost-free runs bit-identical; only set it (e.g. `0.00005` = 万0.5) when a cost-inclusive run is wanted. The benchmark series is not charged.
