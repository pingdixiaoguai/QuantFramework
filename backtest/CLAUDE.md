...
- `runner.py` — day-by-day traversal over the union of all assets' trading days
  ...
  - Optional `config["min_hold_drawdown_threshold"]` is a positive decimal in `(0, 1]`. In `min_hold` mode, a current-position drawdown from its post-entry high at or beyond this threshold releases the lock early, allowing the normal close signal to schedule a next-open rebalance. It does not force an exit when the strategy still selects the current position. The option is disabled by default and does not apply to `fixed_cycle`.
  - A factor config may optionally provide `params_by_asset: {asset_code: {...}}`. The engine overlays the matching asset-specific values on the factor's shared `params` before calling `compute()`. Assets without an override retain the shared parameters.
  - Optional `config["min_hold_factor_unlock"]` can name a registered factor and provide either one `threshold` or per-asset `thresholds`. While in `min_hold`, the engine evaluates that factor for the largest current weight each close and releases the lock when the value is strictly below the applicable threshold.
- `report.py` — lazy-imports `quantstats` and calls `qs.reports.html(...)`; input series receive explicit `Strategy`/benchmark names, and a scoped QuantStats resample wrapper clears pandas frequency metadata so annual bar positions align with year ticks under pandas 2.x
