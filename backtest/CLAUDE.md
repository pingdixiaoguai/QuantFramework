...
- `runner.py` — day-by-day traversal over the union of all assets' trading days
  ...
  - **Rebalance frequency** (`config["rebalance_days"]`, int, default 1): the engine tracks actual open entry dates. While `holding_days < rebalance_days`, it carries the current position without calling `strategy.generate_weights()`; once the window has elapsed, it re-evaluates daily until a different non-empty target is scheduled for the next open.
