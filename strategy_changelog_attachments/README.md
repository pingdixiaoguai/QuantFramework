...
| 2026-08-17 | 动态 ETF 底池（完整历史审计） | 一阶段确认静态/全仓扩池显著降低 Sharpe；二阶段用 2026 存量名单得到 1.331 的候选，但完整历史月末底池审计发现该名单只覆盖历史合格记录的 15.1%。按当时可见规模重建 100 只 ETF 后，原规则仅为 1.275。加入“与核心赢家 60 日相关性≤0.85”的可解释去重门槛后，Sharpe 为 1.289 vs 1.238、5bp 方向仍正、滚动36m领先69%，但 V 段轻微为负且 bootstrap 95% 区间含零。 | **全仓动态池关闭；撤回二阶段存量名单 shadow 结论。相关性门槛规则仅作 paper-shadow 基线，不改生产** | [`最终报告`](2026-08-17_dynamic_etf_pool/2026-08-17_dynamic_etf_pool_phase3_report.md)、[`完整历史设计`](2026-08-17_dynamic_etf_pool/2026-08-17_dynamic_etf_pool_phase3_design.md)、[`二阶段报告（已被审计覆盖）`](2026-08-17_dynamic_etf_pool/2026-08-17_dynamic_etf_pool_phase2_report.md) |
| 2026-08-17 | 固定四ETF的Tushare多字段尾部因子 | 不增加资产，使用成交额、份额申赎、NAV溢价与OHLC尾部字段构建八字段综合风险。结果后候选只在QM20赢家风险≥0.75且为四资产最高时，分50%给池内最低风险资产。全期Sharpe 1.290 vs 1.238，Top10平均回撤−17.69% vs −19.59%，最深回撤−23.64% vs −25.79%，5bp与D/V/T均保持方向；但年化下降1.78pp、候选形成已看过T，Sharpe bootstrap 95%区间含零。 | **达到“Top10降低且Sharpe不跌”的回溯目标；仅为规则冻结的shadow candidate，不改生产** | [`最终报告`](2026-08-17_four_etf_tail_factors/2026-08-17_four_etf_tail_factors_report.md)、[`事前设计`](2026-08-17_four_etf_tail_factors/2026-08-17_four_etf_tail_factors_design.md)、[`稳健性设计`](2026-08-17_four_etf_tail_factors/2026-08-17_four_etf_tail_factors_phase3_design.md) |
| 2026-08-17 | 八字段风险权重拟合 | 用2014—2018未来20日尾部风险拟合非负归一权重，2019—2022单独选择策略参数。拟合权重在V/T的风险预测MSE、IC均优于等权；冻结规则选出T70/W60，全期Sharpe 1.406 vs 1.238、Top10均值改善4.25pp，但最大回撤仅改善0.17pp，未过0.5pp门槛。T70/W50事后通过全部数值门槛，但已看过T。 | **权重拟合有效；正式参数8/9门槛通过仍判失败。T70/W50只作新的paper-shadow备选，不改生产** | [`报告`](2026-08-17_four_etf_tail_factors/2026-08-17_four_etf_tail_factors_phase4_weight_fit_report.md)、[`冻结设计`](2026-08-17_four_etf_tail_factors/2026-08-17_four_etf_tail_factors_phase4_weight_fit_design.md)、[`权重`](2026-08-17_four_etf_tail_factors/2026-08-17_four_etf_tail_factors_phase4_weights.csv) |
| 2026-08-17 | 八字段多机制三目标优化 | 依次检验171组条件分散/换腿、局部稳健性、弱趋势门槛、年度走步元模型、连续预算、高风险换仓延迟和双sleeve。最终事后候选以70%风险延迟Top1+30%轻量SAFE_POS构成；全期Sharpe 1.322 vs 1.238、年化35.17% vs 33.45%、Top10均值−19.24% vs −19.59%、最大回撤−24.73% vs −25.79%，5bp仍正，phase14平台5/8通过。 | **三目标回溯达成，但T已被观察且bootstrap区间跨零；冻结为paper-shadow，不改生产** | [`报告`](2026-08-17_four_etf_tail_factors/2026-08-17_four_etf_tail_factors_phase15_multi_attempt_report.md)、[`最终审计`](2026-08-17_four_etf_tail_factors/2026-08-17_four_etf_tail_factors_phase15_shadow_audit_design.md)、[`shadow规格`](2026-08-17_four_etf_tail_factors/2026-08-17_four_etf_tail_factors_phase15_shadow_spec.yaml) |
...
- **动态扩池边界**：2026-08-17 研究表明，把规模过线且趋势较强的宽基/行业 ETF 临时加入 Top1 全仓排名仍不能提高 Sharpe；全仓动态池关闭。二阶段在 2026 存量名单上形成的 15% 卫星候选存在明显幸存者偏差：该名单只覆盖完整历史合格记录的 15.1%，因此原 shadow 结论撤回。无幸存者偏差的月末底池审计支持一个较弱但可解释的 paper-shadow 基线：20/60/120 日趋势同向、趋势前三、QM20 胜过核心、与核心赢家 60 日相关性不高于 0.85、最多 15% 双卫星、黄金 veto、rd=5。其全期 Sharpe 有约 +0.051 增量，但 V 段略负且 bootstrap 95% 区间含零，不能修改生产。
- **固定池尾部因子候选**：不扩充四资产时，单独用价格风险或Tushare成交/申赎/NAV风险都会牺牲Sharpe；八字段综合风险与QM20形成互补。只有当QM20赢家同时是四资产风险最高且综合分≥0.75时，条件性地将50%仓位分给最低风险资产，回溯上同时改善Sharpe和Top10尾部。该候选是结果后形成，年化收益下降且bootstrap仍不确定，只允许冻结为shadow，不构成生产变更授权。
- **八字段拟合权重**：以未来20日尾部风险为标签、仅在2014—2018拟合后，价格振幅/CVaR/下行波动合计权重升至52.17%，风险预测在V/T均优于等权。冻结验证规则选出的60%分散虽显著提高Sharpe与Top10回撤，却未达到最大回撤门槛；读取T后发现40%—50%更稳健只能视为事后证据。生产不改，若继续只冻结T70/W50做paper-shadow，禁止继续扫参。
- **八字段三目标shadow**：后续多机制研究显示，单独分散会牺牲强趋势收益，单独延迟高风险换仓的改善又集中于少数窗口。将70%资本放入“高风险低置信度新赢家延迟3日”的Top1 sleeve，30%放入正动量轻量安全腿，回溯上同时提高Sharpe、年化并降低Top10平均回撤；5/8相邻sleeve组合跨段通过。但该组合形成时T已被反复观察，年度一致性与bootstrap仍不足，只允许按冻结YAML做paper-shadow，生产保持原QM20。
