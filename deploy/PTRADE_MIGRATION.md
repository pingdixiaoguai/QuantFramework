...
1. **rebalance_days = 2（刻意保留，与框架基准 rd=5 不一致——非配置漂移）**：
   ...
   **2026-07-20 复核，维持 rd=2**：除外部对照外新增理由——rd=2 换仓周期短，
   能更快触发完整换仓，加速 §6.1 异步执行链路的模拟盘验证。
...
**部署记录（2026-07-20）**：本修复版（commit `7395370`）已全文粘贴部署至模拟盘，
`REBALANCE_DAYS = 2` 维持（理由见 §5.1 的 2026-07-20 复核）。模拟盘观察窗从本日重新起算，
上述完整换仓 gate **尚未通过**，通过后在此补记日期与日志摘要。

### 6.2 单笔委托数量上限拆单（2026-07-21）

模拟盘 2026-07-21 首次真正走到建仓即失败：账户约 1000 万，`best=510300.SS`，
`买入委托已提交 ... 目标市值 9800000.00，限价 4.670`，随即
`后端服务 委托数量大于可委托数量【1000000】股，撤销委托` → `买单已终结但未形成持仓`。

**根因**：9,800,000 ÷ 4.670 = **2,098,500 股**，超过交易所**单笔申报数量上限 100 万股**
（沪深两市基金/ETF 限价申报均为 100 万份），后端整单撤销。信号与异步状态机都正常
（状态机正确 fail-closed，未写假持仓）；缺的是**单笔委托数量约束**。全仓 Top1 + 千万级
资金买 4–9 元 ETF，天然需要 >100 万股，无纯配置绕法，必须拆单。

**修复**（买卖两侧都改；仅交易模式，回测路径保持原样以维持 §7/对账口径）：

1. 新增常量 `MAX_ORDER_SHARES = 1_000_000`；`_split_order_shares()` 把超限股数拆成
   每笔 ≤ 上限的整百股子单。
2. **买入** `_submit_trade_buy`：目标市值按限价折算成整百目标股数，减去已持得增量，
   拆成多笔 `order(target, chunk, limit_price=)`（原单笔 `order_target_value` 会超限）。
3. **卖出** `_start_trade_switch`：每只按 `enable_amount` 拆成多笔 `order(sec, -chunk, limit_price=)`
   （原单笔 `order_target(sec, 0)` 在 >100 万股的仓上会同样被撤）。
4. 订单跟踪从单个 `g.pending_buy_order` 改为一组 `g.pending_buy_orders`；`pending_sell_orders`
   的值由单 ID 改为 ID 列表。买入完成条件仍是「真实持仓到位 **且** 本策略无未完成订单」；
   任一子单失败 → `pending_buy_failed` → 次日 `needs_top_up` 补足（复用既有机制）。
5. 回归测试新增 `test_large_buy_splits_into_capped_chunks` /
   `test_large_sell_splits_into_capped_chunks`（`tests/test_ptrade_execution_state.py`，共 10 passed）。

**下单原语假设**：拆单改用按股数的 `order(sec, amount, limit_price=)`（正买负卖）。
这是 PTrade 最基础原语（`order_target*` 均为其上层封装），几乎必然可用；首次部署仍需在
启动/首笔子单日志确认 `order` 调用返回有效 order_id、无「函数不支持」告警。

**⚠️ 单笔上限值需现场确认**：常量取后端此次对沪市 510300 报的 100 万股。沪深两市基金
（ETF）单笔限价申报上限一般都是 100 万份，故对四只 ETF 应都安全；但 159915（深市创业板
ETF）下次轮到买入前，最好在平台确认其单笔上限不低于 100 万。拿不准就把 `MAX_ORDER_SHARES`
调小（如 50 万）—— 只是子单变多，永远安全。

重新部署后须重新观察一次完整换仓：日志应出现「买入子单已提交 …（拆 N 笔子单）」、
无「委托数量大于可委托数量」告警、最终「买入持仓已确认」。

**验证记录（2026-07-21，模拟盘全现金建仓）**：拆单修复版重新部署后，同日建仓日志：

- 信号 `best=510300.SS`；增量 2,074,000 股按限价 4.725 折算，**拆 3 笔子单**（1,000,000 + 1,000,000 + 74,000），每笔 ≤ 100 万。
- 三笔均 `生成订单`、`订单回报 status=2`（已报），**无「委托数量大于可委托数量」告警**；累计全额成交（4.668/4.669，优于限价），`买入持仓已确认: held=510300.SS；needs_top_up=False`。

**已验证** ✅：买入拆单端到端跑通；`order(sec, amount, limit_price=)` 原语在本平台可用（首次部署待确认项已清）；沪市 510300 单笔上限 = 100 万股属实。

**仍待观察** ⏳（本次全现金建仓未覆盖）：
1. **卖出拆单**——无旧仓可卖，未触发；待首次真正换仓验证。
2. **159915（深市）单笔上限**——本次只买沪市 510300，未触碰；信号切到 159915 且需买入时才确认深市 ETF 上限 ≥ 100 万。
3. **§6.1 完整异步换仓 gate**（卖→买→确认三段）——本次缺「卖出委托已提交」段，未走完；rd=2 下建仓满 2 个交易日后若信号切换即触发。
